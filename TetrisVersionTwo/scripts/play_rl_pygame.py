"""Pygame playback for VersionTwo PPO/DQN checkpoints."""
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Play VersionTwo RL policy in pygame.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Auto-reset on game over (default: on).",
    )
    parser.add_argument(
        "--no-auto-reset",
        action="store_false",
        dest="auto_reset",
        help="Stop on game over until reset manually.",
    )
...
def action_name(action_idx: int) -> str:
    return ("None", "Left", "Right", "SoftDrop", "HardDrop", "RotateCW", "RotateCCW", "Hold")[int(action_idx)]
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    runtime = EnvCtypes(lib_path, args.seed)
    obs_dim = runtime.observation_size(include_hidden_rows=False)
    if policy.obs_dim != obs_dim:
        runtime.close()
        raise SystemExit(f"Checkpoint obs_dim={policy.obs_dim} incompatible with env obs_dim={obs_dim}.")
    if policy.action_dim != len(RL_ACTION_MAP):
        runtime.close()
        raise SystemExit(
            f"Checkpoint action_dim={policy.action_dim} incompatible with RL action space={len(RL_ACTION_MAP)}."
        )

    pygame.init()
    pygame.display.set_caption("VersionTwo RL Playback")
    ...
    screen_h = max(560, board_y + board_h + 20)
            ...
            if not paused:
                if meta["game_over"]:
                    ...
                    if args.auto_reset:
                        ...
                        total_reward = 0.0
                        last_reward = 0.0
                        status = f"Topout -> auto-reset seed={seed}"
                    else:
                        paused = True
                        status = "Topout reached. Press R/N or Space."
                    ...
                    if args.auto_reset:
                        ...
                        total_reward = 0.0
                        last_reward = 0.0
                        status = f"Step limit -> auto-reset seed={seed}"
                    else:
                        paused = True
                        status = f"Reached max_steps={args.max_steps}. Press R/N or Space."
                else:
                    obs = runtime.observation(include_hidden_rows=False)
                    action_idx = int(
                        policy.act(
                            obs,
                    ...
                    action_idx = max(0, min(len(RL_ACTION_MAP) - 1, action_idx))
                    out = runtime.step_action(RL_ACTION_MAP[action_idx])
                    step_count += 1
                    last_action = action_idx
                    last_reward = float(out["reward"])
