"""Training entrypoint for custom PPO on VersionTwo CCTetrisEnv."""
...
from TetrisVersionTwo.rl.ppo.core import PPOAgent, PPOConfig, RolloutBuffer


def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train custom PPO on VersionTwo C++ env.")
    ...
    parser.add_argument("--log-dir", type=Path, default=Path("runs/v2_ppo"))
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    obs_batch, _ = env.reset(seed=[args.seed + i for i in range(args.num_envs)])
    ...
    try:
        while global_step < args.total_timesteps:
            ...
            for _ in range(args.n_steps):
                actions, log_probs, values = agent.act_batch(obs_batch)
                ...
                ep_returns += np.asarray(rewards, dtype=np.float32)
