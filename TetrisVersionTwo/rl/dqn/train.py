"""Training entrypoint for custom DQN on VersionTwo CCTetrisEnv."""
...
from TetrisVersionTwo.rl.dqn.core import DQNAgent, DQNConfig, ReplayBuffer
from TetrisVersionTwo.rl.env import CCTetrisEnv


def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train custom DQN on VersionTwo C++ env.")
    ...
    parser.add_argument("--log-dir", type=Path, default=Path("runs/v2_dqn"))
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    agent = DQNAgent(cfg)
    replay = ReplayBuffer(args.buffer_size, obs_dim=obs_dim)
