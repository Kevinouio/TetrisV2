...
void Dag::advance(Piece next_piece, const Placement& placement) {
    root_.advance(next_piece, placement);
    if (!queue_.empty()) {
        queue_.pop_front();
    }
    nodes_.clear();
...
void Dag::add_piece(Piece piece) { queue_.push_back(piece); }
...
DagStatistics Dag::do_work(const FreestyleWeights& weights, double exploitation) {
    ...
    for (int depth_guard = 0; depth_guard < 256; ++depth_guard) {
        ...
        if (edges.empty()) {
            ...
            std::vector<std::pair<Placement, std::uint32_t>> reserve_moves{};
            if (key.state.reserve != Piece::None && key.state.reserve != chosen) {
                reserve_moves = find_moves(key.state.board, key.state.reserve);
            }
            edges.reserve(next_moves.size() + reserve_moves.size());

            auto append_child =
                [&](const std::pair<Placement, std::uint32_t>& move, bool use_hold) {
                    GameState child_state = key.state;
                    const auto info = child_state.advance(chosen, move.first);
                    ...
                    const std::uint16_t next_depth = static_cast<std::uint16_t>(
                        std::min<int>(65535, static_cast<int>(key.depth) + 1));
            ...
            for (const auto& move : next_moves) {
                append_child(move, false);
            }
            for (const auto& move : reserve_moves) {
                append_child(move, true);
            }
            sort_edges(&edges);
            node.expanded = true;
