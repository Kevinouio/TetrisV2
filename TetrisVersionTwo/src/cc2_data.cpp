...
void clear_lines_column(std::uint64_t* col, std::uint64_t lines) {
    while (lines != 0) {
        const unsigned int i = static_cast<unsigned int>(__builtin_ctzll(lines));
        const std::uint64_t mask = (i == 64u) ? ~0ull : ((1ull << i) - 1ull);
...
PlacementInfo GameState::advance(Piece next, const Placement& placement) {
    if (next != Piece::None) {
        bag_mask = static_cast<PieceMask>(bag_mask & ~piece_bit(next));
    }
    if (bag_mask == 0) {
    ...
    if (cleared_mask != 0) {
        ...
        const bool hard =
            (__builtin_popcountll(cleared_mask) == 4) || (placement.spin != Spin::None);
    ...
    out.lines_cleared = static_cast<std::uint32_t>(__builtin_popcountll(cleared_mask));
    out.combo = combo;
