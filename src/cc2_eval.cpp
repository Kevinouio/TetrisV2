...
std::uint32_t height_of_col(std::uint64_t col) {
    return 64u - bit_ops::countl_zero_u64(col);
}

std::uint32_t trailing_ones(std::uint64_t v) {
    return bit_ops::countr_zero_u64(~v);
}
...
FreestyleEvalResult evaluate_freestyle(
    ...
    for (std::size_t i = 0; i < cutout_count; ++i) {
        ...
        const auto lines = bit_ops::popcount_u64(board.line_clears());
    ...
    for (const auto c : state.board.cols) {
        ...
        holes += bit_ops::popcount_u64(hole_bits);
    }
    ...
    for (const auto c : state.board.cols) {
        ...
        while (hole_bits != 0) {
            const auto y = bit_ops::countr_zero_u64(hole_bits);
    ...
    std::uint32_t row_transitions =
        bit_ops::popcount_u64(~state.board.cols[0]) +
        bit_ops::popcount_u64(~state.board.cols[9]);
    for (std::size_t x = 0; x + 1 < state.board.cols.size(); ++x) {
        row_transitions += bit_ops::popcount_u64(state.board.cols[x] ^ state.board.cols[x + 1]);
    }
