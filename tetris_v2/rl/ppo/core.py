...
class ActorCritic(nn.Module):
    """Shared MLP network for policy logits and value."""
...
class PPOAgent:
    ...
    def __init__(self, config: PPOConfig):
        ...
        self.policy = ActorCritic(config.obs_dim, config.action_dim, config.hidden_sizes).to(self.device)
    ...
    def _mask_logits(self, logits: Tensor, action_mask: Optional[np.ndarray | Tensor]) -> Tensor:
        ...
        return logits.masked_fill(~mask, neg_large)
    ...
    def act_batch(
        ...
        *,
        ...
        scaled_logits = self._mask_logits(scaled_logits, action_mask)
        probs = torch.softmax(scaled_logits, dim=-1)
        if epsilon > 0:
            if action_mask is not None:
                legal = torch.from_numpy(action_mask).to(self.device) if isinstance(action_mask, np.ndarray) else action_mask.to(self.device)
                if legal.ndim == 1:
                    legal = legal.unsqueeze(0)
                legal = (legal > 0.5).float()
                counts = legal.sum(dim=-1, keepdim=True).clamp(min=1.0)
    ...
    def update(self, buffer: RolloutBuffer, batch_size: int, epochs: int) -> Dict[str, float]:
        ...
        for _ in range(epochs):
            for batch in buffer.iter_minibatches(batch_size):
                ...
                surr2 = torch.clamp(ratios, 1.0 - self.config.clip_range, 1.0 + self.config.clip_range) * advantages
